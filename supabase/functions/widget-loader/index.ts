...
;(function(w,d,u){
  ...
  function render(cfg) {
    ...
    style.textContent = inIframe ? \`
      ...
      #wj-pop{display:none;width:calc(100% - 24px);max-width:420px;height:calc(100% - 24px);max-height:780px;border-radius:16px;box-shadow:0 10px 40px rgba(0,0,0,.2);overflow:hidden;background:\${bgMain};z-index:2147483647;pointer-events:auto;transform:translateZ(0)}
    ...
    \` : \`
      ...
      #wj-btn svg{width:24px;height:24px}
      #wj-btn img{width:100%;height:100%;object-fit:cover}
      #wj-pop{display:none;position:absolute;bottom:0;\${cfg.widget_position === 'left' ? 'left' : 'right'}:0;width:420px;height:640px;border-radius:16px;box-shadow:0 10px 40px rgba(0,0,0,.2);overflow:hidden;background:\${bgMain};z-index:2147483647;transform:translateZ(0)}
      #wj-pop.open{display:flex;flex-direction:column;animation:wj-expand .35s cubic-bezier(0.16,1,0.3,1)}
      #wj-pop.closing{display:flex;flex-direction:column;animation:wj-collapse .3s cubic-bezier(0.4,0,0.2,1) forwards}
      @keyframes wj-expand{0%{opacity:0}100%{opacity:1}}
      @keyframes wj-collapse{from{opacity:1;transform:scale(1) translateY(0)}to{opacity:0;transform:scale(0.85) translateY(16px)}}
